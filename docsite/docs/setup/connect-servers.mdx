...
Periphery stores its keys, repositories, stacks, builds, SSL material, and File Manager journal beneath `/data` in the `komodo_data` named volume. Docker's host volume root remains mounted separately at the same absolute path so the File Manager can access named volumes.
