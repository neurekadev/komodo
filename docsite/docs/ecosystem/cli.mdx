...
:::note
These database commands use the legacy on-disk Core export format. The encrypted, repository-backed Core/Stack/Volume feature is managed from [Backups](/docs/backups); the legacy API remains available for compatibility.
:::
...
well as a distroless image: **`ghcr.io/neurekadev/komodo-cli`**.
...
<RemoteCodeFile
	title="https://github.com/neurekadev/komodo/blob/main/config/komodo.cli.toml"
	url="https://raw.githubusercontent.com/neurekadev/komodo/main/config/komodo.cli.toml"
	language="toml"
/>
```
