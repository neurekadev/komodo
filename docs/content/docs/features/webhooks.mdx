...
Webhooks are disabled on newly created resources. Each resource keeps its stored setting until an administrator changes it.
...
- You have configured a strong resource-specific webhook secret or a global `KOMODO_WEBHOOK_SECRET` and can store the same value in the repository settings.

1. Open the resource's **Config → Webhooks** section.
2. Set a resource-specific secret, or confirm that Core has a global `KOMODO_WEBHOOK_SECRET`, then enable the webhook and copy its URL.
...
The expected result is one authenticated resource update for each matching event. Requests are rejected when both secret locations are empty or when the effective secret is a shipped placeholder such as `REPLACE_WITH_SECRET`. If delivery fails, inspect the provider response, listener proxy route, signature type, secret, and branch filter.
...
5. Set **Secret** to the resource-specific secret when configured; otherwise use `KOMODO_WEBHOOK_SECRET`.
