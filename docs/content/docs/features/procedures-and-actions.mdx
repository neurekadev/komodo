...
description: Build repeatable multi-stage workflows and API-backed scripts.
---


Komodo offers `Procedure` and `Action` resources for orchestrating multi-resource workflows. Both can be run on a **[schedule](/features/schedules)**

## Create a Repeatable Workflow

### Prerequisites

- Each underlying Build, Stack, Deployment, or other resource operation succeeds on its own.
- The creator can execute every resource included in the workflow.

1. Use a **Procedure** for ordered, declarative resource operations or an **Action** for custom TypeScript logic.
2. Create the resource and add the smallest useful operation.
3. Save and run it manually.
4. Inspect every stage or script line in the update log.
5. Add error handling, more steps, or a schedule only after the first run succeeds.

The expected result is one auditable update containing the complete workflow. If it fails, run the first failing operation directly and confirm the execution user has permission on every referenced resource.
...
| `schedule` | Schedule expression. See [Schedules](/features/schedules). | `""` |
...
Batch executions can also filter by resource tags. When both a name pattern and
tags are configured, Komodo expands the batch only to resources that satisfy
both filters. Leave the tag list empty to retain pattern-only matching.

```toml
[[procedure.config.stage]]
name = "Deploy matching stacks"
executions = [
  { execution.type = "BatchDeployStackIfChanged", execution.params.pattern = "foo-* , \\^bar-.*$\\", execution.params.tags = ["production"] },
]
```
...
The Action editor uses the fork-generated TypeScript client from the same Git
revision as Core.
