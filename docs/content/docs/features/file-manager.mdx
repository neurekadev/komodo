...
File Manager also refuses Stack and Volume roots overlapping a Docker volume labelled `komodo.file-manager.excluded` with any value other than `false`. Official Compose applies this to Core's `backups` and `core-secrets` volumes. The same directory-identity check covers parent, child, bind-mount, and symlink aliases. Docker metadata must be available to validate these exclusions; an unavailable daemon does not bypass them. Keep the labels on custom deployments too, and inspect existing volumes because Docker does not update their labels automatically. This restricts the File Manager API, not a host administrator or a compromised worker with Docker-socket authority.
