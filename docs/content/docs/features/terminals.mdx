---
title: Terminals
description: Open audited shell sessions on servers and in containers.
---
...
- The Server reports **OK**, or the target container is running.
- Your account has Execute access to the target resource.

1. Open the Server, Deployment, or Stack service.
2. Select **Terminal** and choose a new or existing session.
3. Keep **Exec** for a normal container shell; use **Attach** only when you intend to interact with the main process.
4. Run the command and close the client when finished.

The expected result is a persistent, audited session that can be reopened. If the shell exits immediately, try an installed command such as `sh`, verify Execute permission, and check the Periphery connection.
...
Terminal sessions can also be accessed from the command line using the [Komodo CLI](/reference/cli#terminals).
...
For scripted commands, see [Execute Terminal](/reference/api-and-client-libraries#execute-terminal).
