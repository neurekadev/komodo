...
Komodo automatically supplies `IMAGE_NAME` and `VERSION` to the Docker build
when those arguments are not already present in `build_args`. It also supplies
`IMAGE_TAG` when the Build has a non-empty image tag. An explicit value in
`build_args` takes precedence, which lets a Dockerfile use a different internal
value without changing the image tags Komodo publishes.
...
1. Launch an EC2 instance and install Docker:
   ...
   ```
2. If the builder needs custom Periphery configuration, place it at `/etc/komodo/config/periphery.config.toml`. The container mounts that directory read-only at `/config`, where the image loads configuration. An existing `/etc/komodo/periphery.config.toml` must be moved into this directory. Restrict access to the file and any AMI containing credentials; alternatively, provision secrets securely before the container starts.
