...
description: Reuse configuration values without exposing sensitive data.
---


A variable or secret in Komodo is a reusable key-value pair. Variables may be returned to authorized users; secret values are masked after they are stored.

## Create and Use a Value

1. Open **Settings → Variables** and create a variable for non-sensitive data or a secret for credentials.
2. Give it a clear uppercase key, such as `REGISTRY_HOST`.
3. Reference it from a resource field with double brackets, such as `[[REGISTRY_HOST]]`.
4. Save the resource and review the rendered configuration without exposing secret values.
5. Run the resource and verify the application receives the expected value.

The expected result is consistent interpolation across resources. If a key remains literal, check its spelling and brackets; if access is denied, confirm the user and execution have permission to use the value.
...
- **In the UI**, you can go to `Settings` page, `Variables` tab. Here, you can create some Variables to store in the Komodo database.
  ...
  - Variables can also be managed in ResourceSyncs (see [example](/features/resource-sync#deployments)) but should only be done for non-secret variables, to avoid committing sensitive data. You should manage secrets using one of the following options.

- **Mount a config file to Core**: [Advanced Configuration](/administration/core-configuration#mount-a-config-file)
