...
description: Connect Periphery agents and monitor Docker servers.
---



Every server you want to manage with Komodo needs the **Periphery** agent installed.
Periphery is a small, stateless binary that receives commands from Komodo Core
...
3. Read the storage or traffic columns in the resource list, or open an item
   to see its details.

Komodo shows a small animated icon while Periphery completes the first
background measurement. The result then changes to the measured value or to
**Unavailable** with a reason when Docker cannot provide a complete result.
Image and volume storage usage is refreshed every five minutes by default;
set `docker_disk_usage_polling_rate` in `periphery.config.toml` to change that
interval.

If a storage metric remains unavailable, inspect the Periphery logs for a
Docker disk-usage error and run `docker system df -v` on the host. Confirm that
the Periphery user can access Docker and that the command completes. If the UI
shows **The initial Docker disk usage measurement failed**, upgrade Periphery
to a build containing the disk-usage request fix, then restart it. Current
Periphery builds request the complete verbose snapshot and read the image and
volume sections from Docker's response. Docker may take longer on hosts with
many or very large images and volumes.
