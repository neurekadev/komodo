...
This way, it inherits Core's configuration for administrative commands.

Use [Backups](/administration/backups) for supported encrypted Core, Stack, and Volume protection and recovery in 3.0.0.
...
The Core image includes `km`. Use it for administrative commands that should inherit Core's configuration:
...
Komodo does not publish or support native CLI binaries. Advanced users may compile `km` from the repository source for self-supported use; see [Contributing](https://github.com/neurekadev/komodo/blob/main/CONTRIBUTING.md).
...
multiple Cores you want to connect to.
