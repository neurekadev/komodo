---
title: Komodo CLI
description: Configure and use the km command-line client.
---
...
The Komodo CLI, `km`, can be used to:
  ...
  - Perform legacy Database **backup**, **restore**, and **copy**.
  - **Connect to server / container shell** via Komodo Terminals.

The Komodo Core image comes packaged with the Komodo CLI,
and is available for usage inside running container with `docker exec -it komodo km ...`.
This way, it inherits the Core database config in order to easily perform backups with `km db backup -y`.

<Callout title="Legacy database format" type="info">
These database commands use the legacy on-disk Core export format. The encrypted, repository-backed Core/Stack/Volume feature is managed from [Backups](/administration/backups); the legacy API remains available for compatibility.
</Callout>
...
The Core image includes `km`. Use it for database operations or other commands that should inherit Core's configuration:

```bash
docker exec -it komodo km --help
docker exec -it komodo km db backup -y
```

For a standalone CLI, use the official distroless container image. Mount the directory containing your CLI configuration:

```bash
alias km='docker run --rm -it -v "$HOME/.config/komodo:/config" ghcr.io/neurekadev/komodo-cli:3 km'
km config
```

Keep `-it` for interactive `km connect`, `km exec`, and `km attach` sessions: `-i` keeps standard input open, and `-t` supplies the terminal required for raw mode and resizing. For unattended commands, use `docker run --rm` without `-it`, and pass `-y` where confirmation is required.

Komodo does not publish or support native CLI binaries. Advanced users may compile `km` from the repository source for self-supported use; see [Development](/reference/development).
...
```bash
wget -P komodo https://raw.githubusercontent.com/neurekadev/komodo/main/config/komodo.cli.toml
```
