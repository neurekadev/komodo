...
3. Select **Run workflow**, choose your branch, and select **Run workflow** again.
4. Wait for **Lint**, **Unit Tests**, and **Build** to pass for the commit you
   intend to merge.
...
The Rust workspace lists `bin/cli`, `bin/core`, and `bin/periphery` explicitly.
Keep shell test and helper directories such as `bin/tests` outside the Cargo
member list; add a new Rust binary crate to the list when introducing one.
