...
Development Compose persists Core's `/data` in `core-data`, including the default `/data/backups/vykar` repository, Vykar cache, and operational alerts. The separate `core-secrets` volume holds the repository sealing/authentication key; FerretDB keeps its own `data:/state` volume. Preserve all three across rebuilds and do not use `down -v` when retaining development snapshots. A container layer created before adding `core-data` is not copied into that volume automatically: copy any existing `/data` state out before recreating that container.
