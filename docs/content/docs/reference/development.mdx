...
The unpublished Rust workspace packages use the sentinel version `0.0.0`.
Cargo package metadata is not a Komodo release version and does not change for
fork releases or upstream synchronization. If the fork publishes a Rust crate
in the future, give that crate an explicit, independently maintained semantic
version instead of inheriting the workspace sentinel.
