...
The inherited Cargo package version remains `2.3.2`. It identifies package
metadata for the fork's Rust workspace; it is not a Komodo release version and
does not need to change when a Git release is created.
