...
4. Run `docker compose config`, restart only the affected service, and inspect its logs.
5. Verify sign-in, server connectivity, and one read-only operation before making another change.

The expected result is a healthy Core and connected Periphery agents using the new value. If the service fails, restore the previous file, re-run `docker compose config`, and restart it before continuing.

## First Server and Container Routing

`KOMODO_FIRST_SERVER_ADDRESS` is optional. Core now applies legacy database
repairs before creating startup resources, so an old deployment can safely
leave this variable defined while upgrading. After startup, confirm the first
Server appears once and reports **OK**.
