...
1. Keep `KOMODO_LOCAL_AUTH=true` until the external provider has been tested. The Compose examples enable it by default. Setting it to `false` disables local username/password sign-in, including existing local accounts.
...
3. Add the matching environment variables documented in [`core.config.toml`](https://github.com/neurekadev/komodo/blob/main/config/core.config.toml) to `.env`, including the enabled flag, client ID, and client secret. New registration is closed by default: set `KOMODO_DISABLE_OIDC_USER_REGISTRATION=false` for OIDC, or `KOMODO_DISABLE_USER_REGISTRATION=false` for GitHub or Google.
...
To control who can register and whether new accounts need approval, follow [Control registration and default access](/administration/users-groups-permissions#control-registration-and-default-access). Blocking local registration preserves existing local logins, which is useful when new users should register through OIDC. GitHub and Google follow the global registration setting; set the local and OIDC overrides to `true` when opening the global setting only for those named providers.
