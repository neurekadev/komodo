...
description: Grant people and teams the minimum Komodo access they need.
---
...
1. Open **Settings → Users** and enable each approved account.
2. Create a User Group for the team's role instead of granting the same permissions user by user.
3. Add the users to the group.
4. Grant the narrowest base level on the required resources, then add specific permissions such as Logs, Terminal, or File Manager only when needed.
5. Sign in as a test member and verify both an allowed operation and an operation that should be denied.

The expected result is that users see only permitted resources and cannot update or execute beyond the group's grants. If access is surprising, inspect direct user grants, every inherited group, type-wide permissions, and `Everyone` groups before increasing access.
...
For permissioning at scale, users can define [**User Groups in Resource Syncs**](/features/resource-sync#user-group).
...
- **`Backups`**: User can access encrypted snapshots for the associated Stack or Server-backed Volume.
  ...
  - New Server and Stack creators receive `Backups` with their creator permissions. Existing grants are unchanged and must be updated explicitly. Creation does not enroll the Server as a trusted backup worker: an administrator must separately verify its address and Periphery public key in [Trusted backup workers](/administration/backups#enroll-trusted-workers) before Core sends shared repository credentials. Anyone administering an enrolled host must be trusted with fleet-wide repository access.
  - Cross-node restore also requires Execute plus `Backups` on the destination Server. Any recovered Stack requires that destination Server permission—even on its historical Server—plus the existing Stack-create permission.
  - Fleet schedule, repositories, retention, Core recovery, mirror promotion, unbound snapshots, and maintenance remain admin-only. See [Backups](/administration/backups#permissions).
- **`FileManager`**: User can access the [File Manager](/features/file-manager) on the associated resource.
